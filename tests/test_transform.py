...
def _assert_randomness_works(transf, tfmtype=TfmType.PIXEL ):
    ...
    assert not np.all(y4 == y3), "Two distinct calls to determ should return 2 different transformations if it is random"
...
def test_it_is_possible_to_transform_mask_n_coords():
    ...
    assert x.shape[1:] == m.shape[1:], "Mask should have the same shape as img"
